...
def load_file_path_txt_wo_gt(file_path):
    img_list = []
    fov_list = []
    with open(file_path, 'r') as file_to_read:
        while True:
            ...
            img_list.append(img)
            fov_list.append(fov)
    return img_list, fov_list


# Load the original image, grroundtruth and FOV of the data set in order, and check the dimensions
def load_data_wo_gt(data_path_list_file):
    print('\033[0;33mload data from {} \033[0m'.format(data_path_list_file))
    img_list, fov_list = load_file_path_txt_wo_gt(data_path_list_file)
    imgs = None
    FOVs = None
    for i in range(len(img_list)):
        img = np.asarray(readImg(img_list[i]))
...
def load_file_path_txt_wo_fov_gt(file_path):
    img_list = []
    with open(file_path, 'r') as file_to_read:
        while True:
            lines = file_to_read.readline().strip()  # read a line
...
def load_data_wo_fov_gt(data_path_list_file):
    print('\033[0;33mload data from {} \033[0m'.format(data_path_list_file))
    img_list = load_file_path_txt_wo_fov_gt(data_path_list_file)
    imgs = None
    for i in range(len(img_list)):
        img = np.asarray(readImg(img_list[i]))
        imgs = np.expand_dims(img, 0) if imgs is None else np.concatenate((imgs, np.expand_dims(img, 0)))
...
def get_data_test_overlap_wo_gt(test_data_path_list, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original, test_FOVs= load_data_wo_gt(test_data_path_list)
    ...
    return patches_imgs_test, test_imgs_original, test_FOVs, test_imgs.shape[2], test_imgs.shape[3]




def get_data_test_overlap_wo_fov_gt(test_data_path_list, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original= load_data_wo_fov_gt(test_data_path_list)
